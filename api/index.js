// GoHighLevel MCP Integration API for Vercel
export default async function handler(req, res) {
  ...
  async function callGHLMCP(toolName, arguments_) {
    ...
    const mcpUrl = 'https://services.leadconnectorhq.com/mcp/';
  ...
  try {
    ...
    if (method === 'GET' && path === '/api') {
      return res.json({
        ...
        version: '26.0.0',
        description: 'GoHighLevel MCP Server integration for Live Chat',
        status: 'operational',
    ...
    if (method === 'POST' && path === '/api/chat/send') {
      ...
      try {
        ...
        return res.status(500).json({
          success: false,
          error: `GoHighLevel MCP message sending failed: ${mcpError.message}`
        });
      }
    }
